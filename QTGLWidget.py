...
class QTGLWidget(QGLWidget):
    ...
    def captureview(self):
        image_buffer = glReadPixels(0, 0, self.width, self.height, OpenGL.GL.GL_BGR, OpenGL.GL.GL_UNSIGNED_BYTE)
        image = np.frombuffer(image_buffer, dtype=np.uint8).reshape(self.width, self.height, 3)
        image = np.flip(image, axis=0) #image top-down flip

        image_save_path = QFileDialog.getSaveFileName(self, 'Save image', '', "Image Files (*.png);;All Files (*)", options=QFileDialog.DontUseNativeDialog)
        # ('/home/nattaon/onz/opencv_pyqt2/layout_detector_tf_custom2/005line1', 'Image Files (*.png)')
        #print("captureview",len(image_save_path),image_save_path)
        if(len(image_save_path[0])>0):
            image_save_path = image_save_path[0]+".png"
            cv2.imwrite(image_save_path, image)
        #cv2.imwrite("capture3d.png", image)
